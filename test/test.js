...
it('hooks test', async () => {
  ...
  expect(rMap[path.join(seed.path, 'gitignore')]).toEqual([
    path.join(FRAG_PATH, '.gitignore')
  ])
  expect(rMap[path.join(seed.path, 'npmignore')]).toEqual([
    path.join(FRAG_PATH, '.npmignore')
  ])
